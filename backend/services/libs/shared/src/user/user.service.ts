...
export class UserService {
  ...
  async approveUser(company: Company) {
    this.logger.verbose(
      "User approve request received for company",
      company.companyId
    );
        ...
        const result = await this.userRepo
          .update(
            ...
            },
            {
              isPending: false,
              password:
                this.passwordHashService.getPasswordHash(generatedPassword),
            }
          )
          .catch((err: any) => {
            this.logger.error(err);
  ...
  async validateAndCreateUser(
    ...
    this.logger.verbose(
      `User received for validation ${userDto.email} ${companyId}`
    );
    ...
    if (userDto.company) {
      ...
      if (this.configService.get("systemType") !== SYSTEM_TYPE.CARBON_UNIFIED) {
        const companyExists =
          await this.companyService.checkCompanyExistOnOtherSystem({
            taxId: createdUserDto.company?.taxId,
            paymentId: createdUserDto.company?.paymentId,
            email: createdUserDto.company?.email,
          });
        if (companyExists) {
          this.handleDuplicateCompanyError(
            createdUserDto.company,
            companyExists
          );
        }
      }
  ...
  async create(
    ...
    if (company) {
      ...
      const duplicateCompany =
        await this.companyService.checkForCompanyDuplicates(
          company.email,
          company.taxId,
          company.paymentId
        );
  ...
  async download(queryData: DataExportQueryDto, abilityCondition: string) {
    ...
    if (resp.length > 0) {
      ...
      for (const key of titleKeys) {
        headers.push(
          this.helperService.formatReqMessagesString("userExport." + key, [])
        );
      }
