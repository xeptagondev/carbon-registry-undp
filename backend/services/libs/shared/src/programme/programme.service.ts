...
import { ObjectionLetterGen } from "../util/document-generators/objection.letter.gen";
...
import { sectoralScopesMapped } from "../constants/sectoralSecor.mapped";
...
import { LetterOfIntentRequestGen } from "../util/document-generators/letter.of.intent.request.gen";
import { LetterOfIntentResponseGen } from "../util/document-generators/letter.of.intent.response.gen";
...
import { LetterSustainableDevSupportLetterGen } from "../util/document-generators/letter.sustainable.dev.support";
...
export class ProgrammeService {
  ...
  private async doInvestment(
    ...
    if (
      programme.article6trade == true ||
      programme.article6trade == undefined
    ) {
      let nationalProps = {};
    ...
    const savedProgramme = await this.entityManager
      .transaction(async (em) => {
        ...
        (
          await this.checkPendingInvestmentValidity(
            programme,
            transfer,
            nationalInvestment
          )
        ).map(async (toRejectId) => {
          await em.update(
            Investment,
            {
  ...
  async addInvestment(req: InvestmentRequestDto, requester: User) {
    ...
    if (req.percentage) {
      for (const i in req.percentage) {
        req.percentage[i] = this.helperService.halfUpToPrecision(
          req.percentage[i]
        );
      }
    }
    ...
    if (
      req.period &&
      (req.period.length != 2 || req.period[0] >= req.period[1])
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
    ...
    const govProfile = await this.companyService.findGovByCountry(
      this.configService.get("systemCountry")
    );
    if (
      req.fromCompanyIds.includes(govProfile.companyId) &&
      req.percentage[req.fromCompanyIds.indexOf(govProfile.companyId)] !== 0
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
    ...
    for (const j in req.fromCompanyIds) {
      ...
      investment.nationalInvestmentId = req.nationalInvestmentId
        ? req.nationalInvestmentId
        : null;
  ...
  private async getCreditRequest(
    ...
    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.AGRICULTURE &&
    ...
    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.SOLAR &&
      ndcActionDto.subTypeOfMitigation ===
        SubTypeOfMitigation.SOLAR_PHOTOVOLTAICS_PV
    ) {
    ...
    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.SOLAR &&
      ndcActionDto.subTypeOfMitigation ===
        SubTypeOfMitigation.SOLAR_WATER_PUMPING_OFF_GRID
    ) {
      const sr = new SolarWaterPumpingOffGridCreationRequest();
      const solarWaterPumpOff =
        ndcActionDto.creditCalculationProperties as SolarWaterPumpOffGridProperties;
      sr.energyGeneration = solarWaterPumpOff?.energyGeneration;
      sr.energyGenerationUnit = solarWaterPumpOff?.energyGenerationUnit;
      if (constants) {
        sr.solarWaterPumpingOffGridConstants =
          constants.data as SolarWaterPumpingOffGridConstants;
      }
      return sr;
    }
    ...
    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.SOLAR &&
      ndcActionDto.subTypeOfMitigation ===
        SubTypeOfMitigation.SOLAR_WATER_PUMPING_ON_GRID
    ) {
      const sr = new SolarWaterPumpingOnGridCreationRequest();
      const solarWaterPumpOn =
        ndcActionDto.creditCalculationProperties as SolarWaterPumpOnGridProperties;
      sr.energyGeneration = solarWaterPumpOn?.energyGeneration;
      sr.energyGenerationUnit = solarWaterPumpOn?.energyGenerationUnit;
      if (constants) {
        sr.solarWaterPumpingOnGridConstants =
          constants.data as SolarWaterPumpingOnGridConstants;
      }
      return sr;
    }

    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.EE_HOUSEHOLDS &&
      ndcActionDto.subTypeOfMitigation ===
        SubTypeOfMitigation.STOVES_HOUSES_IN_NAMIBIA
    ) {
      const sr = new StovesHousesNamibiaCreationRequest();
      const stoves =
        ndcActionDto.creditCalculationProperties as StovesHousesInNamibiaProperties;
      sr.numberOfDays = stoves?.numberOfDays;
      sr.numberOfPeopleInHousehold = stoves?.numberOfPeopleInHousehold;
      if (constants) {
        sr.stovesHousesNamibiaConstants =
          constants.data as StovesHousesNamibiaConstants;
      }
      return sr;
    }
    ...
    if (
      ndcActionDto.typeOfMitigation === TypeOfMitigation.AGRICULTURE &&
      ndcActionDto.subTypeOfMitigation ===
        SubTypeOfMitigation.SOIL_ENRICHMENT_BIOCHAR
    ) {
      const sr = new SoilEnrichmentCreationRequest();
      const soilEnhancementBiocharProperties =
        ndcActionDto.creditCalculationProperties as SoilEnhancementBiocharProperties;
      ...
      if (req) {
        try {
          ...
          try {
            ...
            ndcAction.ndcFinancing.systemEstimatedCredits =
              this.helperService.halfUpToPrecision(crdts);
          } catch (err) {
  ...
  async approveDocumentCommit(
    ...
    if (
      ...
      if (program && d.type == DocType.METHODOLOGY_DOCUMENT) {
        ...
        if (
          program.article6trade == true ||
          program.article6trade == undefined
        ) {
          await this.asyncOperationsInterface.AddAction({
  ...
  async addDocumentRegistry(documentDto: ProgrammeDocumentRegistryDto) {
    ...
    if (documentDto.certifierTaxId) {
      const certifier = await this.companyService.findByTaxId(
        documentDto.certifierTaxId
      );
    ...
    if (sqlProgram.currentStage != resp.currentStage) {
      if (
        sqlProgram.article6trade == true ||
        sqlProgram.article6trade == undefined
      ) {
        await this.asyncOperationsInterface.AddAction({
          actionType: AsyncActionType.CADTUpdateProgramme,
          actionProps: {
  ...
  async addDocument(documentDto: ProgrammeDocumentDto, user: User) {
    ...
    if (
      ...
      if (
        resp &&
        dr.type === DocType.VERIFICATION_REPORT &&
        dr.status === DocumentStatus.ACCEPTED
      ) {
  ...
  async queueDocument(
    ...
    if (
      ...
      const certifierId = (
        await this.companyService.findByTaxId(req.certifierTaxId)
      )?.companyId;
      await this.programmeLedger.addDocument(
        req.externalId,
        req.actionId,
        req.data,
        req.txTime,
  ...
  async create(
    ...
    if (programmeDto.programmeProperties.geographicalLocation.length > 0) {
      for (const location of programmeDto.programmeProperties
    ...
    if (programme.creditEst)
      programme.creditEst = this.helperService.halfUpToPrecision(
        programme.creditEst
      );
    ...
    if (
      programmeDto.article6trade == true &&
      Number(govProfile.nationalSopValue) !== 0 &&
      !programmeDto.proponentTaxVatId.includes(govProfile.taxId) &&
    ...
    if (programmeDto.article6trade == false) {
      const implementingUser = await this.companyService.findByTaxId(
        programmeDto.implementinguser
      );
        ...
        if (monitoringReport) {
          ...
          if (programmeDto.article6trade == true) {
            await this.queueDocument(
              AsyncActionType.DocumentUpload,
              {
                type: this.helperService.enumToString(
                  DocType,
  ...
  async addNDCAction(
    ...
    const programmeOwnedCompanyIds = program.companyId.map((x) =>
      parseInt(x.toString())
    );
    ...
    if (ndcAction.action === NDCActionType.Mitigation) {
      ...
      if (
        ...
        !mitigationSubTypesListMapped[ndcAction.typeOfMitigation].includes(
          ndcAction.subTypeOfMitigation
        )
      ) {
        throw new HttpException(
    ...
    if (ndcAction.solarProperties?.energyGeneration) {
      ndcAction.solarProperties.energyGeneration =
        this.helperService.halfUpToPrecision(
          ndcAction.solarProperties.energyGeneration
        );
    }
    if (ndcAction.creditCalculationProperties?.energyGeneration) {
      ndcAction.creditCalculationProperties.energyGeneration =
        this.helperService.halfUpToPrecision(
          ndcAction.creditCalculationProperties.energyGeneration
        );
    }
    if (ndcAction.agricultureProperties?.landArea) {
      ndcAction.agricultureProperties.landArea =
        this.helperService.halfUpToPrecision(
          ndcAction.agricultureProperties.landArea
        );
    }
    ...
    if (
      ...
      ndcAction.ndcFinancing.availableCredits =
        ndcAction.ndcFinancing.userEstimatedCredits;
      if (this.configService.get("systemType") == SYSTEM_TYPE.CARBON_UNIFIED) {
        const addMitigationLedger = {
          ...
          systemEstimatedCredits: ndcAction.ndcFinancing?.systemEstimatedCredits
            ? ndcAction.ndcFinancing?.systemEstimatedCredits
            : 0,
          actionId: ndcAction.id,
          constantVersion: "" + ndcAction.constantVersion,
          properties: ndcAction.agricultureProperties
            ? ndcAction.agricultureProperties
            : ndcAction.solarProperties
            ? ndcAction.solarProperties
            : undefined,
        };
        await this.programmeLedger.addMitigation(
          program.externalId,
          addMitigationLedger
        );
      } else {
  ...
  async queryNdcDetails(
    ...
    return new DataListResponseDto(undefined, undefined);
  }
  ...
  async downloadNdcActions(
    ...
    const resp = await queryBuilder
      .orderBy(
        queryDto?.sort?.key &&
          `"ndcaction".${this.helperService.generateSortCol(
            queryDto?.sort?.key
          )}`,
        queryDto?.sort?.order,
  ...
  private prepareNdcActionDataForExport(ndcActions: any) {
    ...
    for (const ndcAction of ndcActions) {
      ...
      const concatenatedReportUrls = ndcActionReports
      ...
      dto.agricultureLandAreaUnit =
        ndcAction.agricultureProperties?.landAreaUnit;
      dto.solarEnergyGeneration = ndcAction.solarProperties?.energyGeneration;
      dto.solarEnergyGenerationUnit =
        ndcAction.solarProperties?.energyGenerationUnit;
      dto.solarConsumerGroup = ndcAction.solarProperties?.consumerGroup;

      dto.creditCalculationTypeOfMitigation =
        ndcAction.creditCalculationProperties?.typeOfMitigation;
      dto.creditCalculationSubTypeOfMitigation =
        ndcAction.creditCalculationProperties?.subTypeOfMitigation;
      dto.creditCalculationEnergyGeneration =
        ndcAction.creditCalculationProperties?.energyGeneration;
      dto.creditCalculationEnergyGenerationUnit =
        ndcAction.creditCalculationProperties?.energyGenerationUnit;
      dto.creditCalculationConsumerGroup =
        ndcAction.creditCalculationProperties?.consumerGroup;
      dto.creditCalculationLandArea =
        ndcAction.creditCalculationProperties?.landArea;
      dto.creditCalculationLandAreaUnit =
        ndcAction.creditCalculationProperties?.landAreaUnit;
      dto.creditCalculationWeight =
        ndcAction.creditCalculationProperties?.weight;
      dto.creditCalculationNumberOfDays =
        ndcAction.creditCalculationProperties?.numberOfDays;
      dto.creditCalculationNumberOfPeopleInHousehold =
        ndcAction.creditCalculationProperties?.numberOfPeopleInHousehold;

      dto.ndcFinancingUserEstimatedCredits =
        ndcAction.ndcFinancing?.userEstimatedCredits;
      dto.ndcFinancingSystemEstimatedCredits =
        ndcAction.ndcFinancing?.systemEstimatedCredits;

      dto.coBenefitsPropertiesSdgGoals =
        ndcAction.coBenefitsProperties?.sdgGoals;
      //coBenefitsProperties:safeGuards
      dto.coBenefitsPropertiesSafeguardsIsRespectHumanRights =
        ndcAction.coBenefitsProperties?.safeguardDetails?.isRespectHumanRights;
      dto.coBenefitsPropertiesSafeguardsIsProjectDiscriminate =
        ndcAction.coBenefitsProperties?.safeguardDetails?.isProjectdiscriminate;
      dto.coBenefitsPropertiesSafeguardsGenderEqualityQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.genderEqualityQ1;
      dto.coBenefitsPropertiesSafeguardsGenderEqualityQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.genderEqualityQ2;
      dto.coBenefitsPropertiesSafeguardsGenderEqualityQ3 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.genderEqualityQ3;
      dto.coBenefitsPropertiesSafeguardsGenderEqualityQ4 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.genderEqualityQ4;
      dto.coBenefitsPropertiesSafeguardsCommunityHealthQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.communityHealthQ1;
      dto.coBenefitsPropertiesSafeguardsHistoricHeritageQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.historicHeritageQ1;
      dto.coBenefitsPropertiesSafeguardsForcedEvictionQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.forcedEvictionQ1;
      dto.coBenefitsPropertiesSafeguardsLandTenureQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.landTenureQ1;
      dto.coBenefitsPropertiesSafeguardsLandTenureQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.landTenureQ2;
      dto.coBenefitsPropertiesSafeguardsIndigenousPeopleQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.indegenousPeopleQ1;
      dto.coBenefitsPropertiesSafeguardsCorruptionQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.corruptionQ1;
      dto.coBenefitsPropertiesSafeguardsLabourRightsQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsQ1;
      dto.coBenefitsPropertiesSafeguardsLabourRightsQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsQ2;
      dto.coBenefitsPropertiesSafeguardsLabourRightsQ3 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsQ3;
      dto.coBenefitsPropertiesSafeguardsLabourRightsQ4 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsQ4;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ1;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ2;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ3 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ3;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ4 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ4;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ5 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ5;
      dto.coBenefitsPropertiesSafeguardsLabourRightsSubQ6 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.labourRightsSubQ6;
      dto.coBenefitsPropertiesSafeguardsEconomicConsequencesQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.economicConsequencesQ1;
      dto.coBenefitsPropertiesSafeguardsEmissionsQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.emissionsQ1;
      dto.coBenefitsPropertiesSafeguardsEnergySupplyQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.energySupplyQ1;
      dto.coBenefitsPropertiesSafeguardsWaterPatternQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.waterPatternQ1;
      dto.coBenefitsPropertiesSafeguardsErosionQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.erosoinQ1;
      dto.coBenefitsPropertiesSafeguardsErosionQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.erosoinQ2;
      dto.coBenefitsPropertiesSafeguardsLandscapeQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.landscapeQ1;
      dto.coBenefitsPropertiesSafeguardsNaturalDisasterQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.naturalDisasterQ1;
      dto.coBenefitsPropertiesSafeguardsGeneticQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.geneticQ1;
      dto.coBenefitsPropertiesSafeguardsPollutantsQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.pollutantsQ1;
      dto.coBenefitsPropertiesSafeguardsHazardousWasteQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.hazardousWasteQ1;
      dto.coBenefitsPropertiesSafeguardsPesticidesQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.pesticidesQ1;
      dto.coBenefitsPropertiesSafeguardsHarvestForestsQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.harvestForestsQ1;
      dto.coBenefitsPropertiesSafeguardsFoodQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.foodQ1;
      dto.coBenefitsPropertiesSafeguardsAnimalHusbandryQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.animalHusbandryQ1;
      dto.coBenefitsPropertiesSafeguardsCriticalHabitatsQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.criticalHabitatsQ1;
      dto.coBenefitsPropertiesSafeguardsEndangeredSpeciesQ1 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.endangeredSpeciesQ1;
      dto.coBenefitsPropertiesSafeguardsEndangeredSpeciesQ2 =
        ndcAction.coBenefitsProperties?.safeguardDetails?.endangeredSpeciesQ2;

      // coBenefitsProperties:AssessmentDetails
      dto.coBenefitsPropertiesAssessmentDetailsEmail =
        ndcAction.coBenefitsProperties?.assessmentDetails?.email;
      dto.coBenefitsPropertiesAssessmentDetailsTitle =
        ndcAction.coBenefitsProperties?.assessmentDetails?.title;
      dto.coBenefitsPropertiesAssessmentDetailsFunder =
        ndcAction.coBenefitsProperties?.assessmentDetails?.funder;
      dto.coBenefitsPropertiesAssessmentDetailsLastName =
        ndcAction.coBenefitsProperties?.assessmentDetails?.lastName;
      dto.coBenefitsPropertiesAssessmentDetailsFirstName =
        ndcAction.coBenefitsProperties?.assessmentDetails?.firstName;
      dto.coBenefitsPropertiesAssessmentDetailsStudyName =
        ndcAction.coBenefitsProperties?.assessmentDetails?.studyName;
      dto.coBenefitsPropertiesAssessmentDetailsTelephone =
        ndcAction.coBenefitsProperties?.assessmentDetails?.telephone;
      dto.coBenefitsPropertiesAssessmentDetailsOrganisation =
        ndcAction.coBenefitsProperties?.assessmentDetails?.organisation;
      dto.coBenefitsPropertiesAssessmentDetailsAffiliationCdm =
        ndcAction.coBenefitsProperties?.assessmentDetails?.affiliationCDM;
      dto.coBenefitsPropertiesAssessmentDetailsVerifyingDetails =
        ndcAction.coBenefitsProperties?.assessmentDetails?.verifyingDetails;
      dto.coBenefitsPropertiesAssessmentDetailsVerifyingOrgName =
        ndcAction.coBenefitsProperties?.assessmentDetails?.verifyingOrgName;
      dto.coBenefitsPropertiesAssessmentDetailsIsThePersonListed =
        ndcAction.coBenefitsProperties?.assessmentDetails?.isThePersonListed;
      dto.coBenefitsPropertiesAssessmentDetailsIsWillingToVerified =
        ndcAction.coBenefitsProperties?.assessmentDetails?.IsWillingToVerified;
      dto.coBenefitsPropertiesAssessmentDetailsIsThirdPartyVerified =
        ndcAction.coBenefitsProperties?.assessmentDetails?.IsThirdPartyVerified;
      // coBenefitsProperties:Economic
      dto.coBenefitsPropertiesEconomicGrowthQ1 =
        ndcAction.coBenefitsProperties?.economic?.growthQ1;
      dto.coBenefitsPropertiesEconomicGrowthQ2 =
        ndcAction.coBenefitsProperties?.economic?.growthQ2;
      dto.coBenefitsPropertiesEconomicGrowthQ3 =
        ndcAction.coBenefitsProperties?.economic?.growthQ3;
      dto.coBenefitsPropertiesEconomicGrowthQ4 =
        ndcAction.coBenefitsProperties?.economic?.growthQ4;
      dto.coBenefitsPropertiesEconomicGrowthQ5 =
        ndcAction.coBenefitsProperties?.economic?.growthQ5;
      dto.coBenefitsPropertiesEconomicGrowthQ6 =
        ndcAction.coBenefitsProperties?.economic?.growthQ6;
      dto.coBenefitsPropertiesEconomicGrowthQ7 =
        ndcAction.coBenefitsProperties?.economic?.growthQ7;
      dto.coBenefitsPropertiesEconomicGrowthQ8 =
        ndcAction.coBenefitsProperties?.economic?.growthQ8;
      dto.coBenefitsPropertiesEconomicEnergyQ1 =
        ndcAction.coBenefitsProperties?.economic?.energyQ1;
      dto.coBenefitsPropertiesEconomicEnergyQ2 =
        ndcAction.coBenefitsProperties?.economic?.energyQ2;
      dto.coBenefitsPropertiesEconomicEnergyQ3 =
        ndcAction.coBenefitsProperties?.economic?.energyQ3;
      dto.coBenefitsPropertiesEconomicEnergyQ4 =
        ndcAction.coBenefitsProperties?.economic?.energyQ4;
      dto.coBenefitsPropertiesEconomicEnergyQ5 =
        ndcAction.coBenefitsProperties?.economic?.energyQ5;
      dto.coBenefitsPropertiesEconomicTechTransferQ1 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ1;
      dto.coBenefitsPropertiesEconomicTechTransferQ2 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ2;
      dto.coBenefitsPropertiesEconomicTechTransferQ3 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ3;
      dto.coBenefitsPropertiesEconomicTechTransferQ4 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ4;
      dto.coBenefitsPropertiesEconomicTechTransferQ5 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ5;
      dto.coBenefitsPropertiesEconomicTechTransferQ6 =
        ndcAction.coBenefitsProperties?.economic?.techTransferQ6;
      dto.coBenefitsPropertiesEconomicBalanceOfPaymentsQ1 =
        ndcAction.coBenefitsProperties?.economic?.balanceOfPaymentsQ1;
      dto.coBenefitsPropertiesEconomicBalanceOfPaymentsQ2 =
        ndcAction.coBenefitsProperties?.economic?.balanceOfPaymentsQ2;
      dto.coBenefitsPropertiesEconomicBalanceOfPaymentsQ3 =
        ndcAction.coBenefitsProperties?.economic?.balanceOfPaymentsQ3;
      ...
      dto.coBenefitsPropertiesGenderParityDiscriminationAgainstGirls =
        ndcAction.coBenefitsProperties?.genderPariy
          ?.descriminationAgainstGirls !== undefined
          ? ndcAction.coBenefitsProperties.genderPariy
              .descriminationAgainstGirls
            ? "YES"
            : "NO"
          : undefined;
      ...
      dto.coBenefitsPropertiesGenderParityHarmfulPracticesAgainstGirls =
        ndcAction.coBenefitsProperties?.genderPariy
          ?.harmfulPracticesAgainstGirls !== undefined
          ? ndcAction.coBenefitsProperties?.genderPariy
              ?.harmfulPracticesAgainstGirls
            ? "YES"
            : "NO"
          : undefined;
      ...
      dto.coBenefitsPropertiesGenderParityEqualRightsToHealthToGirls =
        ndcAction.coBenefitsProperties?.genderPariy
          ?.equealRightsToHealthToGirls !== undefined
          ? ndcAction.coBenefitsProperties?.genderPariy
              ?.equealRightsToHealthToGirls
            ? "YES"
            : "NO"
          : undefined;
      ...
      dto.coBenefitsPropertiesEnvironmentalAirQ2 =
        ndcAction.coBenefitsProperties?.environmental?.airQ2;
      dto.coBenefitsPropertiesEnvironmentalAirQ3 =
        ndcAction.coBenefitsProperties?.environmental?.airQ3;
      dto.coBenefitsPropertiesEnvironmentalAirQ4 =
        ndcAction.coBenefitsProperties?.environmental?.airQ4;
      dto.coBenefitsPropertiesEnvironmentalAirQ5 =
        ndcAction.coBenefitsProperties?.environmental?.airQ5;
      dto.coBenefitsPropertiesEnvironmentalAirQ6 =
        ndcAction.coBenefitsProperties?.environmental?.airQ6;
      dto.coBenefitsPropertiesEnvironmentalAirQ7 =
        ndcAction.coBenefitsProperties?.environmental?.airQ7;
      dto.coBenefitsPropertiesEnvironmentalAirQ8 =
        ndcAction.coBenefitsProperties?.environmental?.airQ8;
      dto.coBenefitsPropertiesEnvironmentalAirQ9 =
        ndcAction.coBenefitsProperties?.environmental?.airQ9;
      dto.coBenefitsPropertiesEnvironmentalLandQ1 =
        ndcAction.coBenefitsProperties?.environmental?.landQ1;
      dto.coBenefitsPropertiesEnvironmentalLandQ2 =
        ndcAction.coBenefitsProperties?.environmental?.landQ2;
      dto.coBenefitsPropertiesEnvironmentalLandQ3 =
        ndcAction.coBenefitsProperties?.environmental?.landQ3;
      dto.coBenefitsPropertiesEnvironmentalLandQ4 =
        ndcAction.coBenefitsProperties?.environmental?.landQ4;
      dto.coBenefitsPropertiesEnvironmentalLandQ5 =
        ndcAction.coBenefitsProperties?.environmental?.landQ5;
      dto.coBenefitsPropertiesEnvironmentalLandQ6 =
        ndcAction.coBenefitsProperties?.environmental?.landQ6;
      dto.coBenefitsPropertiesEnvironmentalLandQ7 =
        ndcAction.coBenefitsProperties?.environmental?.landQ7;
      dto.coBenefitsPropertiesEnvironmentalLandQ8 =
        ndcAction.coBenefitsProperties?.environmental?.landQ8;
      dto.coBenefitsPropertiesEnvironmentalWaterQ1 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ1;
      dto.coBenefitsPropertiesEnvironmentalWaterQ2 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ2;
      dto.coBenefitsPropertiesEnvironmentalWaterQ3 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ3;
      dto.coBenefitsPropertiesEnvironmentalWaterQ4 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ4;
      dto.coBenefitsPropertiesEnvironmentalWaterQ5 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ5;
      dto.coBenefitsPropertiesEnvironmentalWaterQ6 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ6;
      dto.coBenefitsPropertiesEnvironmentalWaterQ7 =
        ndcAction.coBenefitsProperties?.environmental?.waterQ7;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ1 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ1;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ2 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ2;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ3 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ3;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ4 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ4;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ5 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ5;
      dto.coBenefitsPropertiesEnvironmentalNaturalResourceQ6 =
        ndcAction.coBenefitsProperties?.environmental?.naturalResourceQ6;
      // coBenefitsProperties:SocialValue
      dto.coBenefitsPropertiesSocialValueJobRelatedMainQ =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedMainQ;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ1 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ1;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ2 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ2;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ3 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ3;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ4 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ4;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ5 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ5;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ6 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ6;
      dto.coBenefitsPropertiesSocialValueJobRelatedSubQ7 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.jobRelatedSubQ7;
      dto.coBenefitsPropertiesSocialValueHealthRelatedMainQ =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedMainQ;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ1 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ1;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ2 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ2;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ3 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ3;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ4 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ4;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ5 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ5;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ6 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ6;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ7 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ7;
      dto.coBenefitsPropertiesSocialValueHealthRelatedSubQ8 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.healthRelatedSubQ8;
      dto.coBenefitsPropertiesSocialValueEducationRelatedMainQ =
        ndcAction.coBenefitsProperties?.socialValueDetails?.educationRelatedMainQ;
      dto.coBenefitsPropertiesSocialValueEducationRelatedSubQ1 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.educationRelatedSubQ1;
      dto.coBenefitsPropertiesSocialValueEducationRelatedSubQ2 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.educationRelatedSubQ2;
      dto.coBenefitsPropertiesSocialValueEducationRelatedSubQ3 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.educationRelatedSubQ3;
      dto.coBenefitsPropertiesSocialValueEducationRelatedSubQ4 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.educationRelatedSubQ4;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedMainQ =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedMainQ;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ1 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ1;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ2 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ2;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ3 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ3;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ4 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ4;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ5 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ5;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ6 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ6;
      dto.coBenefitsPropertiesSocialValueWelfareRelatedSubQ7 =
        ndcAction.coBenefitsProperties?.socialValueDetails?.welfareRelatedSubQ7;
  ...
  async transferReject(req: ProgrammeTransferReject, approver: User) {
    ...
    if (result.affected > 0) {
      if (pTransfer.isRetirement) {
        ...
        const omgeCredits =
          pTransfer.retirementType == RetireType.CROSS_BORDER
            ? Number(
                (
                  (pTransfer.creditAmount * pTransfer.omgePercentage) /
                  100
                ).toFixed(2)
              )
            : 0;
  ...
  private prepareTransferDataForExport(transfers: any) {
    ...
    for (const transfer of transfers) {
      ...
      dto.status =
        transfer.status === TransferStatus.NOTRECOGNISED
          ? "Not Recognised"
          : transfer.status;
      ...
      dto.certifier =
        transfer.certifier &&
        Array.isArray(transfer.certifier) &&
        transfer.certifier.length > 0
          ? transfer.certifier.map((certifier) =>
              certifier ? certifier.companyId : null
            )
          : [];
      ...
      if (transfer.isRetirement) {
        ...
        const omgeCredits =
          transfer.retirementType == RetireType.CROSS_BORDER
            ? Number(
                (
                  (transfer.creditAmount * transfer.omgePercentage) /
                  100
                ).toFixed(2)
              )
            : 0;
  ...
  private async checkPendingInvestmentValidity(
    programme: Programme,
    toDoInvestment: Investment,
    nationalInvestment?: Investment
  ) {
    ...
    for (var investment of projectInvestments) {
      if (toDoInvestment.requestId !== investment.requestId) {
        const ownerIndex = programme.companyId
          .map((e) => Number(e))
          .indexOf(Number(investment.fromCompanyId));
  ...
  private checkPendingTransferValidity = async (programme: Programme) => {
    ...
    for (let transfer of transfers) {
      ...
      const creditBalance = this.helperService.halfUpToPrecision(
        ...
        if (result.affected === 0) {
          ...
          if (transfer.isRetirement) {
            ...
            await this.emailHelperService.sendEmailToOrganisationAdmins(
              ...
              {
                credits: this.helperService.halfUpToPrecision(
                  transfer.creditAmount - omgeCredits
                ),
  ...
  async transferCancel(req: ProgrammeTransferCancel, requester: User) {
    ...
    if (result.affected > 0) {
      ...
      const omgeCredits =
        transfer.retirementType == RetireType.CROSS_BORDER
          ? Number(
              ((transfer.creditAmount * transfer.omgePercentage) / 100).toFixed(
                2
              )
            )
          : 0;
  ...
  async transferRequest(req: ProgrammeTransferRequest, requester: User) {
    ...
    if (!req.companyCredit) {
      req.companyCredit = programme.creditOwnerPercentage.map((p, i) =>
        this.helperService.halfUpToPrecision(
          this.helperService.halfUpToPrecision(
    ...
    for (const j in req.fromCompanyIds) {
      ...
      if (req.fromCompanyIds.length == 1 && !req.companyCredit) {
        ...
        transferCompanyCredit = this.helperService.halfUpToPrecision(
          req.companyCredit[j]
        );
      }
    ...
    let resp = await this.programmeViewRepo
      ...
      .leftJoinAndMapMany(
        ...
        "programmeDocument.programmeId = programme.programmeId AND programmeDocument.type IN (:...types)", // Adding the condition for types
        {
          types: [
            DocType.DESIGN_DOCUMENT,
            DocType.NO_OBJECTION_LETTER,
            DocType.METHODOLOGY_DOCUMENT,
            DocType.AUTHORISATION_LETTER,
            DocType.ENVIRONMENTAL_IMPACT_ASSESSMENT,
          ],
        }
      )
      .orderBy(
        queryDto?.sort?.key &&
          `"programme".${this.helperService.generateSortCol(
            queryDto?.sort?.key
          )}`,
        queryDto?.sort?.order,
        queryDto?.sort?.nullFirst !== undefined
  ...
  private prepareProgrammeDataForExport(programmes: any) {
    ...
    for (const programme of programmes) {
      ...
      const concatenatedNames = companies
        .map((company) => company?.name)
        .join(", ");

      const certifiers: Company[] = programme.certifier;
      const concatenatedCertifiersNames = certifiers
        .map((company) => company?.name)
        .join(", ");
      ...
      dto.financingType = this.addSpaces(
        programme.programmeProperties.sourceOfFunding
      );
      ...
      dto.carbonPriceUSDPerTon =
        programme.programmeProperties?.carbonPriceUSDPerTon;
      dto.geographicalLocation =
        programme.programmeProperties?.geographicalLocation;
      dto.estimatedProgrammeCostUSD =
        programme.programmeProperties?.estimatedProgrammeCostUSD;
      ...
      dto.creditUpdateTime = this.helperService.formatTimestamp(
        programme.creditUpdateTime
      );
      dto.statusUpdateTime = this.helperService.formatTimestamp(
        programme.statusUpdateTime
      );
      dto.certifiedTime = this.helperService.formatTimestamp(
        programme.certifiedTime
      );
      dto.txRef = programme.txRef;
      dto.txType = programme.txType;
      dto.geographicalLocationCordintes =
        programme.geographicalLocationCordintes;
      dto.environmentalAssessmentRegistrationNo =
  ...
  async issueProgrammeCredit(req: ProgrammeMitigationIssue, user: User) {
    ...
    program.mitigationActions.map((action) => {
      if (
        action.projectMaterial &&
        this.isVerfiedMitigationAction(action.projectMaterial)
      ) {
    ...
    req.issueAmount.map((action) => {
      ...
      if (
        action.issueCredit >
        verfiedMitigationMap[action.actionId].availableCredits
      ) {
        throw new HttpException(
          this.helperService.formatReqMessagesString(
      ...
      verfiedMitigationMap[action.actionId].issuedCredits =
        this.helperService.halfUpToPrecision(
          verfiedMitigationMap[action.actionId].issuedCredits +
            action.issueCredit
        );
      totalCreditIssuance = this.helperService.halfUpToPrecision(
        totalCreditIssuance + action.issueCredit
      );
      this.updateMitigationProps(
        program.mitigationActions,
        action.actionId,
        verfiedMitigationMap[action.actionId]
      );
    });
    ...
    let updated: any = await this.programmeLedger.issueProgrammeStatus(
      ...
      `${this.getUserRefWithRemarks(
        user,
        req.comment
      )}#${this.getNdcCreditIssuanceRef(req.issueAmount)}`,
      program.mitigationActions
    );
    if (!updated) {
      return new BasicResponseDto(
  ...
  async issueCredit(issue: ProgrammeMitigationIssue, abilityCondition: string) {
    ...
    const ndcQuery: QueryDto = {
      page: 1,
      size: 10,
      filterAnd: [{ key: "status", operation: "=", value: NDCStatus.APPROVED }],
      sort: { key: "txTime", order: "DESC" },
      filterBy: undefined,
      filterOr: [
        { key: "action", operation: "=", value: NDCActionType.CrossCutting },
        { key: "action", operation: "=", value: NDCActionType.Mitigation },
      ],
    };
    const approvedMitigationActions = await this.queryNdcActions(
      ndcQuery,
      abilityCondition
    );
    let verfiedMitigationMap = {};
  ...
  async authorizeProgramme(
    req: ProgrammeApprove,
    user: User,
    auth_letter?: string
  ) {
    this.logger.log(
        ...
        auth_letter
          ? await this.emailHelperService.sendEmailToOrganisationAdmins(
              ...
              EmailTemplates.PROGRAMME_AUTHORISATION,
              {
                ...
                programmePageLink:
                  hostAddress +
                  `/programmeManagement/view/${updated.programmeId}`,
              },
              undefined,
              undefined,
              undefined,
  ...
  private getNdcCreditIssuanceRef = (
    issueAmount: mitigationIssueProperties[]
  ) => {
    let ref = "";
    issueAmount.map((action) => {
      ref += `${action.actionId}?${this.helperService.halfUpToPrecision(
        action.issueCredit
      )}&`;
    });
  ...
  async investmentCancel(req: InvestmentCancel, requester: User) {
    ...
    if (investment.initiatorCompanyId != requester.companyId) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
  ...
  async investmentApprove(req: InvestmentApprove, approver: User) {
    ...
    let nationalInvestment = investment.nationalInvestmentId
      ? await this.investmentRepo.findOneBy({
    ...
    if (
      nationalInvestment &&
      nationalInvestment.category !== InvestmentCategoryEnum.National
    ) {
      throw new HttpException(
        this.helperService.formatReqMessagesString(
          "programme.investmentNotAnNationalInvestment",
    ...
    investment.shareFromOwner = parseFloat(
      (
        ...
        propPerMap[investment.fromCompanyId]
      ).toFixed(6)
    );
  ...
  async addNdcDetailsPeriod(
    ndcDetailsPeriod: NdcDetailsPeriodDto,
    ...
    const addedNdcDetailsPeriod =
      this.ndcDetailsPeriodRepo.create(ndcDetailsPeriod);
    await this.ndcDetailsPeriodRepo
      .save(addedNdcDetailsPeriod)
      .catch((error) => {
  ...
  async finalizeNdcDetailsPeriod(
    id: number,
    abilityCondition: any,
    user: User
  ) {
  ...
  async addNdcDetailAction(
    ndcDetailsAction: NdcDetailsActionDto,
    abilityCondition: any,
    user: User
  ) {
    ...
    const addedNdcDetailsAction =
      this.ndcDetailsActionRepo.create(ndcDetailsAction);
    await this.ndcDetailsActionRepo
      .save(addedNdcDetailsAction)
      .catch((error) => {
        this.logger.error(error);
        throw new HttpException(
          this.helperService.formatReqMessagesString(
  ...
  async updateNdcDetailsAction(
    ...
    const result = await this.ndcDetailsActionRepo
      .update(
        {
          id: ndcDetailsAction.id,
        },
  ...
  async approveNdcDetailsAction(
    ...
    await this.ndcDetailsActionRepo
      .update(idDto.id, { status: NdcDetailsActionStatus.Approved })
      .catch((error) => {
        this.logger.error(error);
        throw new HttpException(
          this.helperService.formatReqMessagesString(
  ...
  async rejectNdcDetailsAction(
    ...
    await this.ndcDetailsActionRepo
      .update(idDto.id, { status: NdcDetailsActionStatus.Rejected })
      .catch((error) => {
        this.logger.error(error);
        throw new HttpException(
          this.helperService.formatReqMessagesString(
